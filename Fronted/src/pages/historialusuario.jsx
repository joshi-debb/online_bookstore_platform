...
const HistorialUsuario = () => {
  ...
  const setupdatestate = async (order_id) => {
    try {
      await axios.put(`http://localhost:3000/api/orders/sttDelivered/${order_id}`);
    //recargar pagina
        ...
        {sales.map((sale, index) => (
          <Card className="mb-4" key={index}>
            <Card.Body>
              ...
              {sale.status === 'sent' && (
                <Button variant="primary"  onClick={() => setupdatestate(sale._id)} className="mt-3">Confirmar</Button>
              )}
            </Card.Body>
          </Card>
        ))}
